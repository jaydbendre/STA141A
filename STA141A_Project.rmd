...
>>>>>>> 7a1118f9830c946575cd88a37d665cfacac907f6
```
...
ggplot(plt_dt, aes(x=CVDTESTRSLT, y=cnt, fill=CVDTESTRSLT)) +
	...
	geom_text(label = percent(plt_dt$prcnt, accuracy = 0.1), 
...
ggplot(plt_dt2, aes(fill=CVDTESTRSLT, y=count, x=CVDTESTRSLT)) + 
	...
	geom_text(label = percent(plt_dt2$prcnt, accuracy = 0.1), position = position_dodge(width=0.9), vjust=-0.25) +
...
ggplot(sample_df, aes(fill=CVDTEST, y=count, x=CVDTEST)) + 
	...
	scale_y_continuous(name = "Count of Respondents", labels = scales::comma_format(), expand = expansion(mult = c(0, .2))) +
	geom_text(label = percent(sample_df$prcnt, accuracy = 0.1), position = position_dodge(width=0.9), vjust=-0.25) +
...
ggplot(plt_dt3, aes(fill=CVDTEST, y=count, x=CVDTEST)) + 
	geom_bar(position="dodge", stat="identity", color = "black") +
...
df %>% 
	group_by(CVDDIAG_CD=CVDDIAG, CVDDIAG=as_factor(CVDDIAG)) %>% 
	summarize(Count = n())
...
ggplot(strat_plt, aes(x=CVDDIAG, y=cnt)) +
	...
	theme_classic() + 
	geom_text(label = percent(strat_plt$prc, accuracy = 0.1), position = position_dodge(width=0.9), vjust=-0.25) +
	scale_x_discrete("Ever Told had or Likely had Coronavirus", labels = c("No","Yes")) + 
	theme(legend.position="none",
							text = element_text(family = "Times New Roman"),
							aspect.ratio = 1/2,
							axis.text = element_text(size = 13),
...
idx = sample.int(n = nrow(strat_sample), size = floor(0.70 * nrow(strat_sample)), replace = F)

lr_train <- strat_sample[idx,]
lr_test <- strat_sample[-idx,]
